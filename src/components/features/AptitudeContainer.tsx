            ...
            <div className="min-h-[80vh] px-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
                    {CATEGORIES.map((cat, i) => {
                        const categoryTopics = processedData[cat.id] || [];
                        const done = categoryTopics.filter(t => isTopicDone(t.id)).length;
                        return (
                            <motion.button key={cat.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                                ...
                                <p className="text-sm text-zinc-400 mb-4">{categoryTopics.length} topics</p>
                                <div className="flex items-center justify-between">
                                    <div className="w-full bg-white/10 rounded-full h-1.5 mr-4">
                                        <div className={`h-1.5 rounded-full bg-${cat.color}-400 transition-all`} style={{ width: `${categoryTopics.length > 0 ? (done / categoryTopics.length) * 100 : 0}%` }} />
                                    </div>
